...
const StockRow = ({ stock }) => {
  ...
  return (
    ...
    >
      <td style={{ fontWeight: 700, color: '#ffffff' }}>{stock.symbol}</td>
      <td style={{ color: '#aaaaaa' }}>{stock.name}</td>
      ...
      <td style={{ textAlign: 'right', color: '#888888' }} className="mono-price">${stock.dailyHigh.toFixed(2)}</td>
      <td style={{ textAlign: 'right', color: '#888888' }} className="mono-price">${stock.dailyLow.toFixed(2)}</td>
    </tr>
  );
};
